...
namespace TMPro
{
    ...
    public abstract class TMP_Text : MaskableGraphic
    {
        ...
        /// <summary>
        /// Event delegate to allow custom loading of Material when using the <material="Material Asset Name"> tag.
        /// </summary>
        public static event Func<string, Material> OnMaterialAssetRequest;
        /// <summary>
                ...
                switch (m_xmlAttribute[0].nameHashCode)
                {
                    ...
                    case 28511: // <FONT>
                        ...
                        else if (materialAttributeHashCode == 103415287 || materialAttributeHashCode == 72669687) // using material attribute
                        {
                            ...
                            {
                                // Load new material
                                string materialFile = new string(m_htmlTag, m_xmlAttribute[0].valueStartIndex, m_xmlAttribute[0].valueLength);

                                tempMaterial = Resources.Load<Material>(TMP_Settings.defaultFontAssetPath + materialFile);

                                if (tempMaterial == null)
                                {
                                    tempMaterial = OnMaterialAssetRequest?.Invoke(materialFile);
                                }
                                if (tempMaterial == null)
                                    return false;
