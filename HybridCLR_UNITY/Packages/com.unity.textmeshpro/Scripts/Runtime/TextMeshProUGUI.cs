...
namespace TMPro
{
    ...
    public partial class TextMeshProUGUI : TMP_Text, ILayoutElement, IPointerClickHandler
    {
        /// <summary>
        ...
        */
        ...
        {
            string intersectID = string.Empty;
            int linkIndex = TMP_TextUtilities.FindIntersectingLink(this, eventData.position, eventData.enterEventCamera);
            if (linkIndex > -1)
            {
                TMP_LinkInfo linkInfo = textInfo.linkInfo[linkIndex];
                intersectID = linkInfo.GetLinkID();
            }
            if (!string.IsNullOrEmpty(intersectID))
            {
                PEvent?.Invoke(intersectID, eventData.position);
            }
        }
        private Action<string, Vector3> PEvent;

        public void OnPointerClickEvent(Action<string, Vector3> ed)
        {
            PEvent = ed;
        }
    }
