    ...
    public class UIForm : MonoBehaviour, IUIForm
    {
        ...
        public void OnInit(int serialId, string uiFormAssetName, IUIGroup uiGroup, bool pauseCoveredUIForm, bool isNewInstance, object userData)
        {
            ...
            if (!isNewInstance)
            {
                ...
                //jit   覆盖 aot
            var type = GameEnter.HotFix.GetType(uiFormAssetName);

            if (type == null)
            {
                type = Utility.Assembly.GetType(uiFormAssetName);
            }
            if (type != null)
            {
                m_UIFormLogic=Activator.CreateInstance(type) as UIFormLogic;
            }
