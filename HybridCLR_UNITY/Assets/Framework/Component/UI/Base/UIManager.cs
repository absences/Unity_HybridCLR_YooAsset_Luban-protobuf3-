...
namespace GameFramework.UI
{
    ...
    internal sealed partial class UIManager : IUIManager
    {
        ...
        {
            foreach (var uiGroup in m_UIGroups.Values)
            {
                IUIForm uiForm = uiGroup.GetUIForm(serialId);
                if (uiForm != null)
                {
                    return uiForm;
                }
        ...
        public IUIForm GetUIForm(string uiFormAssetName)
        {
            ...
            foreach (var uiGroup in m_UIGroups.Values)
            {
                IUIForm uiForm = uiGroup.GetUIForm(uiFormAssetName);
                if (uiForm != null)
                {
                    return uiForm;
                }
        ...
        public int OpenUIForm(string uiFormAssetName, string uiGroupName, uint priority, bool pauseCoveredUIForm, object userData)
        {
            ...
            if (uiFormInstanceObject == null)
            {
               ...
               GameEnter.Resource.LoadAssetAsync<GameObject>
                    (uiFormAssetName, priority, m_LoadAssetCallbacks, OpenUIFormInfo.Create(serialId, uiGroup, pauseCoveredUIForm, userData));
            }
            else
            {
                InternalOpenUIForm(serialId, uiFormAssetName, uiGroup, uiFormInstanceObject.Target as GameObject, pauseCoveredUIForm, false, 0f, userData);
            }

            return serialId;
        }
        public UniTask<UIFormLogic> OpenUIFormAsync(string uiFormAssetName, string uiGroupName, uint priority, bool pauseCoveredUIForm, object userData)
        {
            ...
            if (uiFormInstanceObject == null)
            {
                ...
                m_UIFormsBeingLoaded.Add(serialId, uiFormAssetName);
                async UniTask<UIFormLogic> Load()
                {
                    var handle = GameEnter.Resource.LoadAssetAsync<GameObject>(uiFormAssetName, 0);
                    ...
                    if (m_UIFormsToReleaseOnLoad.Contains(serialId))
                    {
                        ...
                        m_UIFormHelper.ReleaseUIForm(handle, null);
                    ...
                    else
                    {
                        ...
                        UIFormInstanceObject uiFormInstanceObject =
                            UIFormInstanceObject.Create(uiFormAssetName, handle,
                            m_UIFormHelper.InstantiateUIForm(handle), m_UIFormHelper);
                        m_InstancePool.Register(uiFormInstanceObject, true);


                        IUIForm uiForm = m_UIFormHelper.CreateUIForm(uiFormInstanceObject.Target as GameObject, uiGroup, userData);
            ...
            else
            {
                ...
                uiForm.OnInit(serialId, uiFormAssetName, uiGroup, pauseCoveredUIForm, false, userData);
                uiGroup.AddUIForm(uiForm);
                uiForm.OnOpen(userData);
                uiGroup.Refresh();
                ...
                if (m_OpenUIFormSuccessEventHandler != null)
                {
                    OpenUIFormSuccessEventArgs openUIFormSuccessEventArgs = OpenUIFormSuccessEventArgs.Create(uiForm, 0, userData);
                    m_OpenUIFormSuccessEventHandler(this, openUIFormSuccessEventArgs);
                    ReferencePool.Release(openUIFormSuccessEventArgs);
                }
                async UniTask<UIFormLogic> Load()
        ...
        private void InternalOpenUIForm(int serialId, string uiFormAssetName, UIGroup uiGroup, GameObject uiFormInstance, bool pauseCoveredUIForm, bool isNewInstance, float duration, object userData)
        {
            try
            {
        ...
        private void LoadAssetSuccessCallback(string uiFormAssetName, HandleBase uiFormAsset, float duration, object userData)
        {
            ...
            InternalOpenUIForm(openUIFormInfo.SerialId, uiFormAssetName, openUIFormInfo.UIGroup, uiFormInstanceObject.Target as GameObject, openUIFormInfo.PauseCoveredUIForm, true, duration, openUIFormInfo.UserData);
            ReferencePool.Release(openUIFormInfo);
        }

        private void LoadAssetFailureCallback(string uiFormAssetName, EOperationStatus status, string errorMessage, object userData)
        {
            OpenUIFormInfo openUIFormInfo = (OpenUIFormInfo)userData;
            if (openUIFormInfo == null)
            {
