...
public class TimerComponent : BaseGameComponent
{
    /// <summary>
    /// timer 类型
    /// </summary>
    ...
    /// <returns></returns>
    public async UniTask<bool> OnceTimerAsync(long time, TimerCancellationToken cancellationToken = null)
    {
        long nowTime = TimerTimeUtility.Now();
        if (time <= 0)
        {
        ...
        try
        {
            cancellationToken.CallBack += (CancelAction);
    ...
    public async UniTask<bool> FrameAsync(TimerCancellationToken cancellationToken = null)
    {
...
public class TimerCancellationToken
{
    ...
    /// 等待时间后执行取消m_Actions
    /// </summary>
    /// <param name="afterTimeCancel"></param>
    public async void CancelAfter(long afterTimeCancel)
    {
        await GameEnter.Timer.OnceTimerAsync(afterTimeCancel);
        Invoke();
    }
}
