...
namespace Telepathy
{
    ...
    class ClientConnectionState : ConnectionState
    {
        ...
        public ClientConnectionState(int MaxMessageSize, object userData) : base(new TcpClient(), MaxMessageSize)
        {
    ...
    public class Client : Common
    {
        ...
        public object UserData => state != null ? state.m_UserData : null;
...

        {
            ...
            catch (SocketException exception)
            {
                ...
                Log.Info("Client Recv: failed to connect to ip=" + ip + " port=" + port + " reason=" + exception.Message);
            }
        ...
        static void ReceiveThreadFunction(ClientConnectionState state, IPAddress iPAddress, int port, int MaxMessageSize, bool NoDelay, int SendTimeout, int ReceiveTimeout, int ReceiveQueueLimit)

        {
            ...
            {
                // connect (blocking)
                state.client.Connect(iPAddress, port);
        ...
        public void Connect(string ip, int port, object userData)
        {
            ...
            state = new ClientConnectionState(MaxMessageSize, userData);
        ...
        public void Connect(IPAddress iPAddress, int port, object userData)
        {
            ...
            state.receiveThread = new Thread(() =>
            {
                ReceiveThreadFunction(state, iPAddress, port, MaxMessageSize, NoDelay, SendTimeout, ReceiveTimeout, ReceiveQueueLimit);
            });
